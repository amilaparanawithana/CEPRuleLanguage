...
public class SiddhiQLConverterImpl extends AbstractConverter implements SiddhiQLConverter {
    ...
    private String createSQLWithQuery(Query query) {
        ...
        query.getFrom().getStreams().forEach(stream -> {
            ...
            queryString.append(QueryKeyWords.SPACE).append(QueryKeyWords.JOIN).append(QueryKeyWords.SPACE);

        });

        queryString.setLength(queryString.length() - 5);
        ...
        if(query.getLimit() != null && !query.getLimit().isEmpty()) {
            queryString.append(" output last ").append(query.getLimit()).append(QueryKeyWords.SPACE);
        }
